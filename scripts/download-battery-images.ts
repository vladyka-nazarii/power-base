...
const seedFiles = ["bak", "cbak", "envision", "eve", "tenpower"].map(
  (manufacturer) => ({
    manufacturer,
    path: path.join(
      process.cwd(),
...
const outputRoot = path.join(process.cwd(), "downloads", "battery");
...
for (const seedFile of seedFiles) {
  ...
  const sourceUrls = [
    ...new Set(
      [...source.matchAll(primaryImagePattern)]
        .map((match) => match[1])
        .filter((sourceUrl) =>
          sourceUrl.startsWith("https://www.nkon.nl/media/catalog/product/"),
        ),
    ),
  ];
